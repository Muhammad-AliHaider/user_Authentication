...
var CacheNamePrefix = 'sw-precache-v1--' + (window.self.registration ? window.self.registration.scope : '') + '-';
...
var mappings = populateCurrentCacheNames(PrecacheConfig, CacheNamePrefix, window.self.location);
...
window.self.addEventListener('install', function(event) {
  event.waitUntil(
    ...
    Promise.all(Object.keys(CurrentCacheNamesToAbsoluteUrl).map(function(cacheName) {
      ...
      if (typeof window.self.skipWaiting === 'function') {
...
if (window.self.clients && (typeof window.self.clients.claim === 'function')) {
  window.self.addEventListener('activate', function(event) {
    event.waitUntil(window.self.clients.claim());
  });
}

window.self.addEventListener('message', function(event) {
...
window.self.addEventListener('fetch', function(event) {
  if (event.request.method === 'GET') {
    ...
    if (!cacheName && navigateFallback && event.request.headers.has('accept') &&
        ...
        /* eslint-enable quotes, comma-spacing */
      var navigateFallbackUrl = new URL(navigateFallback, window.self.location);
